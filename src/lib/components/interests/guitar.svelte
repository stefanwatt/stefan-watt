<script>
  import '@justinribeiro/lite-youtube'
</script>
<div class="card bg-primary-content text-primary">
    <lite-youtube videoid="jqADI3U-vpU"></lite-youtube>
    <div class="card-body">
      <h2 class="card-title text-2xl md:text-3xl">Playing Guitar 🎸 </h2>
      <p class="text-lg md:text-xl mb-4">
        This is me playing Devil on my Shoulder from Billy Talent.
      </p>
      <p class="mt-4">
        I started playing about 4 years ago. I'm self taught as with most my other skills/passions.
      </p>
      <p>
        Juicy solo starts @ <a class="link" target="_blank" rel="noopener noreferrer" href="https://youtu.be/jqADI3U-vpU?t=37">0:37</a> 😉
      </p>
    </div>
    <div class="flex flex-col w-full">
    </div>
  </div>